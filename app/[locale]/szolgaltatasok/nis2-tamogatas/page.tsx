...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const t = await getTranslations({ locale, namespace: "servicePages.nis2" });
    return {
        title: t("metaTitle"),
        description: t("metaDescription"),
        keywords: t.has("keywords") ? (t("keywords") as string).split(", ") : undefined,
        alternates: {
            canonical: `https://sironic.eu/${locale}/szolgaltatasok/nis2-tamogatas`,
        },
    };
}

export default async function Nis2TamogatasPage({ params }: Props) {
    const { locale } = await params;
    const t = await getTranslations({ locale, namespace: "servicePages.nis2" });
    ...
    const jsonLd = {
        ...
        "@type": "Service",
        name: locale === "hu" ? "NIS2-kompatibilis IT-rendszer kiépítése" : "NIS2-compliant IT System Implementation",
    ...
    return (
        <>
            <Script id="nis-json-ld" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
            ...
            <section className={`${styles.hero} bg-grid`}>
                <div className="container">
                    <SectionReveal>
                        ...
                        <span className="badge"><ShieldCheck size={13} />{t("badge")}</span>
                        ...
                        <div style={{ display: "flex", gap: "1rem", marginTop: "2rem", flexWrap: "wrap" }}>
                            <Link href={freeAssessmentHref} className="btn btn-primary btn-lg">
                                {locale === "hu" ? "NIS2 Állapotfelmérés" : "NIS2 Assessment"} <ArrowRight size={16} />
                            </Link>
            ...
            <section className="section">
                <div className="container">
                    <div className="grid-2" style={{ alignItems: "start", gap: "3rem" }}>
                        ...
                        <SectionReveal>
                            <div className="card" style={{ background: "var(--surface-2)", borderColor: "rgba(230,57,70,0.2)" }}>
                                <p style={{ fontSize: "0.875rem", fontWeight: 600, color: "var(--primary)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "1rem" }}>
                                    {locale === "hu" ? "Fontos határidő" : "Important deadline"}
                                </p>
                                <p className="body-lg">
                                    {locale === "hu"
                                        ? "A NIS2-irányelv 2024-től fokozatosan kötelező Magyarországon. Az érintett vállalkozásoknak minél hamarabb érdemes megkezdeni a felkészülést – a késlekedés kockázatot jelent."
                                        : "The NIS2 directive is becoming mandatory in Hungary from 2024. Affected businesses should start preparation as soon as possible – delays carry risk."}
                                </p>
                            </div>
                        </SectionReveal>
                    </div>
